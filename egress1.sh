...
for i in {1..200};
do oc new-project test$i;oc create -f /home/sninganu/egress/verification-tests/testdata/networking/list_for_pods.json;oc get pods;oc label ns test$i department=qe;
done
...
for i in {1..100}; do echo pod$i=mypod;mypod=$(oc get pods -n test$i|awk 'NR==2{print $1}');echo $mypod;echo $test$i;oc project test$i;egress=$(oc exec $mypod -- curl 10.0.13.150:9095);echo $egress;done

for i in {101..200}; do echo pod$i=mypod;mypod=$(oc get pods -n test$i|awk 'NR==2{print $1}');echo $mypod;echo $test$i;oc project test$i;egress=$(oc exec $mypod -- curl 10.0.13.150:9095);echo $egress;done
